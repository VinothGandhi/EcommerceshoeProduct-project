			...
			<table id="prod">
			   ...
			   List<cart> cartList=cartdao.showCart(user);
				...
				<tr>
					...
					<td><a href="orderpro.jsp?user=<%=user %>&brandname=<%=cartList1.getProduct().getBrandName()%>&btype=<%=cartList1.getProduct().getBrandType()%>&bsize=<%=cartList1.getProduct().getBrandSize()%>&color=<%=cartList1.getProduct().getColor()%>"><button><strong>Order</strong></button></a>
